...
/// Returns capture info (dimensions) and spawns a thread that sends frames to `tx`.
pub async fn start_capture(tx: mpsc::Sender<DisplayUpdate>) -> Result<CaptureInfo> {
...
fn capture_thread(
    ...
    // Roundtrip to get buffer constraints (buffer_size, shm_format, done)
    event_queue
        .roundtrip(&mut state)
        .context("failed to get buffer constraints")?;

    // Sometimes needs another roundtrip for all events
    event_queue
        .roundtrip(&mut state)
    ...
    let stride = width * 4; // 4 bytes per pixel for xrgb8888/argb8888
    ...
    // Send capture info back
    let _ = info_tx.send(Ok(CaptureInfo { width, height }));
    ...
    loop {
        ...
        // Attach buffer and set full damage
        ...
        // Dispatch events until frame is ready or failed
        loop {
            event_queue
        ...
        // Destroy the frame object (required by protocol before creating next frame)
        frame.destroy();
        ...
        // Send frame (blocking until channel has space)
...
impl AppState {
    fn new(tx: mpsc::Sender<DisplayUpdate>) -> Self {
        Self {
            tx,
            shm: None,
            output: None,
...
// wl_display and wl_callback are handled by wayland-client internally
