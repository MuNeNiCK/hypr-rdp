...
- **TLS** — Auto-generated self-signed RSA-2048 certificates, or bring your own. An existing
  `~/.config/hypr-rdp/cert.pem` is reused as it is, so a pair generated by an older version keeps
  whatever key it has; delete both files to get a new one.
